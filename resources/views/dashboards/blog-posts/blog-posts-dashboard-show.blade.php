        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-night-light overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('blog-posts-dashboard.update', $post->slug) }}">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            ...
                            <div>
                                ...
                                <textarea id="content" name="content" rows="15" required
                                    class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-night-grey dark:text-gray-300 focus:border-star-cyan focus:ring-star-cyan rounded-md shadow-sm">{{ old('content', $post->content) }}</textarea>
                                    @error('content')
                                        <p class="text-red-900">{{ $message }}</p>
                                    @enderror
                            </div>
                            ...
                            <div>
                                <label class="inline-flex items-center">
                                    <input type="checkbox" name="published" {{ $post->published ? 'checked' : '' }}
                                        class="rounded border-gray-300 text-star-cyan focus:ring-star-cyan">
                                    <span class="ml-2 text-sm text-gray-600 dark:text-gray-300">Publicado</span>
                                </label>
                                @error('published')
                                    <p class="text-red-900">{{ $message }}</p>
                                @enderror
