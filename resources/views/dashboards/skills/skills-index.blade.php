            ...
            <div class="bg-white dark:bg-night-light overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="flex justify-between items-center mb-6">
                        <form method="GET" action="{{ route('skills-dashboard.index') }}"
                        ...
                        <a href="{{ route('skills-dashboard.create') }}"
                                ...
                                @forelse($skills as $skill)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <a href="{{ route('skills-dashboard.show', $skill->id) }}"
                                                class="text-sky-600 hover:text-sky-400 mr-3">Editar</a>
                                            <form action="{{ route('skills-dashboard.destroy', $skill->id) }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="text-red-700 hover:text-red-300"
                                                    onclick="return confirm('¿Realmente quieres eliminar esta habilidad?')">Eliminar</button>
                                            </form>
                                        </td>
                                    </tr>
                                @empty
